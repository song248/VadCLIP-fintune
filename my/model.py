...
class CLIPVAD(nn.Module):
    ...
    def encode_textprompt(self, text):
        ...
        with torch.no_grad():
            word_embedding = self.clipmodel.token_embedding(word_tokens).to(dtype)

        text_embeddings = self.text_prompt_embeddings(torch.arange(77).to(self.device)).to(dtype)
        text_embeddings = text_embeddings.unsqueeze(0).repeat(len(text), 1, 1)
        text_tokens = torch.zeros(len(text), 77).to(self.device)

        for i in range(len(text)):
            ind = (word_tokens[i] != 0).sum() - 1
        ...
        # position embedding도 float16
        x = text_embeddings + self.clipmodel.positional_embedding.to(self.device, dtype=dtype)
        x = x.permute(1, 0, 2).to(dtype)
        x = self.clipmodel.transformer(x)  # 내부적으로 float16
        x = x.permute(1, 0, 2)

        x = self.clipmodel.ln_final(x).type(self.clipmodel.text_projection.dtype)
