...
def test(model, testdataloader, maxlen, prompt_text, gt, gtsegments, gtlabels, device):
    ...
    gt_len = len(gt)
    ap1_rep = np.repeat(ap1, 16)[:gt_len]
    ap2_rep = np.repeat(ap2, 16)[:gt_len]

    # AUC, AP 계산
    ROC1 = roc_auc_score(gt, ap1_rep)
    AP1 = average_precision_score(gt, ap1_rep)
    ROC2 = roc_auc_score(gt, ap2_rep)
    AP2 = average_precision_score(gt, ap2_rep)
...
if __name__ == '__main__':
    ...
    args = my_ucf_option.parser.parse_args()
